...
public class gbEasy {
   ...
   * @param manyMonsters
   */
  public gbEasy(int manyBoxes, int manyCoins, int manyMonsters) {
  ...
  private void boxesInitialization() {
    ...
    coinDiamond diamond = new coinDiamond(50);
    monsSquirtel squirtle = new monsSquirtel(-5, "Squirtle");
    ...
    for (int i = 0; i < manyCoins; i++) {
      boardGame[mixCoin[i]].addCoin(diamond);
    }

    for (int i = 0; i < manyMonsters; i++) {
      boardGame[mixMonsters[i]].addMonster(squirtle);
    }
  }
